...
### mask
**paddingMask**<br>
padding mask只会应用在self-attention的计算中，通过mask将相应位置的权重设置为极小值，表明不会关注到此位置的信息；<br>
具体来讲就是计算完scaled_attention_logits = matmul_qk / tf.math.sqrt(dk) 这一步之后，再softmax之前，将padding位置的值变为极小值
```
if mask is not None:
...
```
paddingMask 会应用在编码层和解码层所有的self-attention中
