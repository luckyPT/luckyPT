...
...
```
个人理解：对于其他Map，如果将K,V放入到其他Map中，只要这个Map不被回收，那么这个K,V便不会被回收，即使在其他任何地方都没有引用。但是如果放到WeakHashMap中，通过K,V构造的是弱引用的Entity，当K在外部没有引用时，这时候Map的Entity是可回收的。这种特性一般用于缓存场景。
